...
s = InstaPy(username=insta_username, password=insta_password,
        headless_browser=False, use_firefox=True)
...
with smart_run(s):
    ...
    s.follow_by_tags(HASHTAGS, amount=2)
    s.unfollow_users(amount=1, allFollowing=True, style="FIFO",
            unfollow_after=30*60*60, sleep_delay=600)
